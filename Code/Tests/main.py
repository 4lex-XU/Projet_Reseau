...
for trame in Trames :
    ...
    if(lectureEthernet(trame) != None):
        ...
        if(type != "0800"):
            ...
            #AJOUT DANS LES TABLEAUX SOURCE ET DESTINATION 
            Tab_PortSrc.append((MAC_dec(MACsrc), "??"))
            Tab_PortDest.append((MAC_dec(MACdst), "??"))
            ...
            if(protocol != "06"):
                ...
                #AJOUT DANS LES TABLEAUX SOURCE ET DESTINATION 
                Tab_PortSrc.append((IPv4_dec(IPSrc), "??"))
                Tab_PortDest.append((IPv4_dec(IPDest), "??"))
                ...
                if(REQUEST == 1):
                    if(premier):
                        ...
                        if((port_dec(PortSrc) == port1 and TotalLength-THL*4 < MSS1) or (port_dec(PortSrc) == port2 and TotalLength-THL*4 < MSS2)):
                            ...
                            Tab_Comment.append(Comment_Frag)
                            FRAG = 1
                            REQUEST = 0
                            SN_Frag = 0
                            AN_Frag = 0
